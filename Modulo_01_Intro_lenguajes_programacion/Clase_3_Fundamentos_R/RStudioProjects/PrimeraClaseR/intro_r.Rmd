...
Podemos mostrar cualquier resultado en la consola utilizando la función *built-in* `print`
...
Como podemos observar, el array ha convertido ambos valores al tipo más general, en este caso el tipo `character`, ya que con una cadena podemos también especificar números. Este proceso se denomina **coerción** y es importante tenerlo en cuenta ya que podemos perder información.
...
Quizás la operación más común en un array es la **indexación**, que aplicamos para acceder a valores individuales del array. Para ello utilizaremos el operador de corchetes `[]`
...
> **Es muy importante tener el cuenta que en R a diferencia de la mayoría de lenguajes, los índices de un vector empiezan desde el 1 y no desde el 0**
...
Hemos visto como almacenar y operar con datos básicos, no obstante, en estadística y en análisis de datos queremos trabajar con valores que representen variables o conjuntos de datos. Un tipo de variable muy común son las variables *categóricas*, es decir, variables cuyo dominio esta compuesto por una serie de valores discretos.
